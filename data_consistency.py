...
class data_consistency():
    ...
    def __init__(self, sens_maps, mask):
        with tf.name_scope('EncoderParams'):
            ...
            self.mask = mask
            self.shape_list = tf.shape(mask)
            self.scalar = tf.complex(tf.sqrt(tf.cast(self.shape_list[0] * self.shape_list[1], tf.float32)), 0.)
...
def conj_grad(input_elems, mu_param):
    ...
    def body(i, rsold, x, r, p, mu):
        with tf.name_scope('CGIters'):
            ...
            alpha = tf.complex(rsold / tf.cast(tf.reduce_sum(tf.math.conj(p) * Ap), tf.float32), 0.)
            x = x + alpha * p
            r = r - alpha * Ap
    ...
    cg_out = tf.while_loop(cond, body, 
                           loop_vars,
                           shape_invariants=(
                                            tf.TensorShape([]),                 # i
