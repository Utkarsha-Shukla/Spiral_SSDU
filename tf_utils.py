...
def tf_complex2real(input_data):
    ...
    return tf.compat.v1.stack([tf.compat.v1.real(input_data), tf.compat.v1.imag(input_data)], axis=-1)
...
def tf_real2complex(input_data):
    ...
    return tf.compat.v1.complex(input_data[..., 0], input_data[..., 1])
...
def tf_fftshift_flip2D(input_data, axes=1):
    ...
    if axes == 1:
        ...
        first_half = tf.compat.v1.identity(input_data[:, :nx, :])
        second_half = tf.compat.v1.identity(input_data[:, nx:, :])

    elif axes == 2:

        first_half = tf.compat.v1.identity(input_data[:, :, :ny])
        second_half = tf.compat.v1.identity(input_data[:, :, ny:])

    else:
        raise ValueError('Invalid axes for fftshift')

    return tf.compat.v1.concat([second_half, first_half], axis=axes)
